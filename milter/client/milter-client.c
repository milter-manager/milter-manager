...
gboolean
milter_client_send_io (MilterClient *client,
                       gint msg_fd,
                       GIOChannel *channel,
                       GError **error)
{
#if FD_PASSING_BY_MSG_CONTROL || FD_PASSING_BY_MSG_ACCRIGHTS
    gint fd;
    struct msghdr msg;
    struct iovec vec[1];
    char buf[1];
    ...
    if (sendmsg(msg_fd, &msg, 0) == -1) {
        g_set_error(error, MILTER_CLIENT_ERROR,
                    MILTER_CLIENT_ERROR_UNIX_SOCKET,
                    "[client][error][send_io] "
                    "sendmsg: %s",
                    g_strerror(errno));
        return FALSE;
    }
    ...
    g_set_error(error, MILTER_CLIENT_ERROR,
                MILTER_CLIENT_ERROR_UNIX_SOCKET,
                "[client][error][send_io] "
                "sendmsg: passing fd is not supported");
    return FALSE;
#endif
}
...
GIOChannel *
milter_client_recv_io (MilterClient *client,
                       gint msg_fd,
                       GError **error)
{
#if FD_PASSING_BY_MSG_CONTROL || FD_PASSING_BY_MSG_ACCRIGHTS
    ...
    if (recvmsg(msg_fd, &msg, 0) == -1) {
        g_set_error(error, MILTER_CLIENT_ERROR,
                    MILTER_CLIENT_ERROR_UNIX_SOCKET,
                    "[client][error][recv_io] "
                    "recvmsg: %s",
                    g_strerror(errno));
        return NULL;
    }
    ...
    if (msg.msg_controllen < (socklen_t)sizeof(struct cmsghdr)) {
        g_set_error(error, MILTER_CLIENT_ERROR,
                    MILTER_CLIENT_ERROR_UNIX_SOCKET,
                    "[client][error][recv_io] "
                    "file descriptor was not passed "
                    "(msg_controllen=%d smaller than sizeof(struct cmsghdr)=%d)",
    ...
    if (msg.msg_controllen < (socklen_t)CMSG_LEN(sizeof(int))) {
        g_set_error(error, MILTER_CLIENT_ERROR,
                    ...
                    "[client][error][recv_io] "
                    "file descriptor was not passed "
                    "(msg_controllen=%d smaller than CMSG_LEN(sizeof(int))=%d)",
                    (int)msg.msg_controllen, (int)CMSG_LEN(sizeof(int)));
        return NULL;
    }
    if ((socklen_t)CMSG_SPACE(sizeof(int)) < msg.msg_controllen) {
        g_set_error(error, MILTER_CLIENT_ERROR,
                    MILTER_CLIENT_ERROR_UNIX_SOCKET,
                    "[client][error][recv_io] "
                    "file descriptor was not passed "
                    "(msg_controllen=%d bigger than CMSG_SPACE(sizeof(int))=%d)",
                    (int)msg.msg_controllen, (int)CMSG_SPACE(sizeof(int)));
        return NULL;
    }
    if (cmsg.hdr.cmsg_len != CMSG_LEN(sizeof(int))) {
	discard_cmsg_resource(&msg);
        g_set_error(error, MILTER_CLIENT_ERROR,
                    MILTER_CLIENT_ERROR_UNIX_SOCKET,
                    "[client][error][recv_io] "
                    "file descriptor was not passed "
                    "(cmsg_len=%d, %d expected)",
